...
async function submitQuery() {
    ...
    const additionalFilters = document.getElementById('filterInput').value.trim();
    ...
    if (regionFilter) {
        filtersObj.customer_region = regionFilter;
    }
    
    // Parse additional filters if provided
    if (additionalFilters) {
        try {
            const pairs = additionalFilters.split(',');
            pairs.forEach(pair => {
                const [key, value] = pair.split('=');
                if (key && value) {
                    filtersObj[key.trim()] = value.trim().replace(/^["']|["']$/g, '');
                }
            });
        } catch (e) {
            console.warn('Warning: Could not parse additional filters:', e);
        }
    }
